...
private:
    ...
    status_t linear_execution(const grid_struct_t &grid_struct);

    status_t cell_execution(const cell_struct_t &cell_struct);
    ...
    std::function<status_t(const cell_struct_t &)> cell_func;
    std::function<status_t(const grid_struct_t &)> grid_func;
