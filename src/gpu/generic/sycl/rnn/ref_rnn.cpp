...
static status_t init_sycl(const rnn_pd_t *rnn_pd, const rnn_utils::conf_t &rnn,
    ...
    off.src_layer = get_outer_strides(rnn_pd->src_md(0));
    off.src_iter = get_outer_strides(rnn_pd->src_md(1));

    off.weights_layer = get_outer_strides(rnn_pd->weights_md(0));
    off.weights_iter = get_outer_strides(rnn_pd->weights_md(1));

    off.bias = get_outer_strides(rnn_pd->weights_md(2));

    off.dst_layer = get_outer_strides(rnn_pd->dst_md(0));
    off.dst_iter = get_outer_strides(rnn_pd->dst_md(1));
...
inline status_t init_sycl(const rnn_utils::conf_t &rnn, const rnn_pd_t *rnn_pd,
        rnn_offsets_t &off) {
    return init_sycl(rnn_pd, rnn, off);
}
...
status_t _ref_rnn_common_t::pd_t::init(impl::engine_t *engine) {
    ...
    VDISPATCH_RNN_SC(init_sycl(rnn_conf, this, this->off), "init_sycl()");
    ...
    copy_init_layer_conf_.range = rnn_conf.slc;
    ...
    copy_init_layer_conf_.dst_md = xpu::sycl::md_t(&state_md);
    copy_init_layer_conf_.layer = true;
    copy_init_layer_conf_.to_state = true;

    copy_init_iter_conf_ = sycl_rnn_copy_conf_t();
    copy_init_iter_conf_.batch = rnn_conf.mb;
    copy_init_iter_conf_.range = rnn_conf.sic;
    ...
    copy_init_iter_conf_.src_md = xpu::sycl::md_t(this->src_md(1));
    copy_init_iter_conf_.dst_md = xpu::sycl::md_t(&state_md);
    copy_init_iter_conf_.layer = false;
    copy_init_iter_conf_.to_state = true;

    copy_res_layer_conf_ = sycl_rnn_copy_conf_t();
    copy_res_layer_conf_.batch = rnn_conf.mb;
    copy_res_layer_conf_.range = rnn_conf.dhc;
    ...
    copy_res_layer_conf_.src_md = xpu::sycl::md_t(&state_md);
    copy_res_layer_conf_.dst_md = xpu::sycl::md_t(this->dst_md(0));
    copy_res_layer_conf_.layer = true;
    copy_res_layer_conf_.to_state = false;

    copy_res_iter_conf_ = sycl_rnn_copy_conf_t();
    copy_res_iter_conf_.range = rnn_conf.dhc;
    ...
    copy_res_iter_conf_.src_md = xpu::sycl::md_t(&state_md);
    copy_res_iter_conf_.dst_md = xpu::sycl::md_t(this->dst_md(1));
    copy_res_iter_conf_.layer = false;
    copy_res_iter_conf_.to_state = false;
...
status_t _ref_rnn_common_t::init(impl::engine_t *engine) {
    ...
    switch (pd()->cell_kind()) {
        case dnnl_vanilla_rnn:
            cell_func = [this](const cell_struct_t &cell_struct) -> status_t {
                return this->cell_execution(cell_struct);
            };
            break;
        default: break;
    }
    grid_func = [this](const grid_struct_t &grid_struct) -> status_t {
...
status_t _ref_rnn_common_t::linear_execution(const grid_struct_t &grid_struct) {
...
status_t _ref_rnn_common_t::execute_(const exec_ctx_t &ctx) const {
    ...
    const grid_struct_t &grid_struct {
